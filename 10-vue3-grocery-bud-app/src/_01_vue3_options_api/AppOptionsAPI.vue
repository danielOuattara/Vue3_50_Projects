...
export default {
  ...
  methods: {
    retrieveLocalStorage() {
      ...
      if (itemsList) {
        return JSON.parse(localStorage.getItem("itemsList-vue3-optionsAPI"));
      } else {
        return [];
      }
    },
    ...
    handleSubmitItem() {
      if (!this.itemName) {
        ...
        this.itemsList = this.itemsList.map((obj) => {
    ...
    editItem(id) {
      const editingItem = this.itemsList.find((item) => item.id === id);
      this.isEditing = true;
      this.editID = id;
